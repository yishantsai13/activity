  ...
  <table width="927" border="0" align="center">
    <tr>
      <td width="449" valign="top"><div id="file_upload_section">
		...
		<div align="center"><font color="#FF1C19"><i>單一檔案上傳限制為2M</i></font></div>
      ...
      <input name="activity_id" type="hidden" value="<?php echo $_GET['activity_id'];?>" />
        ...
        <table id="file_list" width="441" border="0" align="center">
          ...
          <?php 
	$result=mysql_query("select * from hai_active_activity_file where activity_id='".$_GET['activity_id']."'");
	while($row=mysql_fetch_array($result)){
	?>
          <tr>
            <td>
            <?php if($row['ftype']=='jpg' || $row['ftype']=='gif' || $row['ftype']=='png'){?>
            <input type="radio" name="cover_fid" value="<?php echo $row['fid']; ?>" <?php if($row['cover']){ echo 'checked';}?> />
             <?php } ?> 
              </td>
            <td><a target="_blank" href="<?php echo $row['path'];?>"><?php echo $row['fname'];?></a></td>
            <td><?php echo $row['ftype']; ?></td>
            <td><a  name="del_file" href="manage_active_file_do.php?del_file=1&amp;fid=<?php echo $row['fid']; ?>&amp;activity_id=<?php echo $_GET['activity_id'];?>">刪除</a></td>
          </tr>
          <?php } ?>
        </table>
        <p align="center">
          <input type="submit"  id="button_set_cover" value="設為海報" />
        </p>
      </form></td>
    </tr>
  </table>
  <p align="center">
    <input type="button" value="完成活動編輯"  id="end_button" onclick="location='manage.php'"/>
  </p>
</div>
</body>
</html>
